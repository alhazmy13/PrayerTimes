...
public class PrayerTime implements GlobalVariables{
    ...
    public PrayerTime() {
        ...
        methodParams.put(Calculation.Jafari, Jvalues);

        // Karachi
        double[] Kvalues = {18,1,0,0,18};
        methodParams.put(Calculation.Karachi, Kvalues);

        // ISNA
        double[] Ivalues = {15,1,0,0,15};
        methodParams.put(Calculation.ISNA, Ivalues);

        // MWL
        double[] MWvalues = {18,1,0,0,17};
        methodParams.put(Calculation.MWL, MWvalues);

        // Makkah
        double[] MKvalues = {18.5,1,0,1,90};
        methodParams.put(Calculation.Makkah, MKvalues);

        // Egypt
        double[] Evalues = {19.5,1,0,0,17.5};
        methodParams.put(Calculation.Egypt, Evalues);

        // Tehran
        double[] Tvalues = {17.7,0,4.5,0,14};
        methodParams.put(Calculation.Tehran, Tvalues);

        // Custom
        double[] Cvalues = {18,1,0,0,17};
        methodParams.put(Calculation.Custom, Cvalues);
    ...
    private void setCustomParams(double[] params) {
        ...
        for (int i = 0; i < 5; i++) {
            if (params[i] == -1) {
                ...
                methodParams.put(Calculation.Custom, params);
            } else {
                methodParams.get(Calculation.Custom)[i] = params[i];
            }
        }
        this.setCalcMethod(Calculation.Custom);
    }
    ...
    private double[] adjustTimes(double[] times) {
        ...
        if (this.getAdjustHighLats() != Adjusting.None) {
    ...
    private double nightPortion(double angle) {
        ...
        if (adjustHighLats == Adjusting.AngleBased)
            calc = (angle)/60.0;
        else if (adjustHighLats == Adjusting.MidNight)
            calc = 0.5;
        else if (adjustHighLats == Adjusting.OneSeventh)
            calc = 0.14286;
    ...
    // Set time offsets
    public void setOffsets(int[] offsetTimes) {
